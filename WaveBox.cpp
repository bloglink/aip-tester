...
void WaveBox::ShowWave(QVariant msg)
{
    WaveInit = msg;
    WaveItem.clear();
    WaveByte.clear();
    WaveTest.clear();
    QVariantHash hash = msg.toHash();
...
void WaveBox::paintEvent(QPaintEvent *e)
{
    ...
    if (!WaveInit.toHash().value("Block0").toString().isEmpty()) {
        int b0 = WaveInit.toHash().value("Block0").toString().toInt()*this->width()/400;
        painter->setPen(QPen(Qt::yellow, 1, Qt::SolidLine));
        painter->drawLine(QPoint(b0, 0), QPoint(b0, height()));
    }
    if (!WaveInit.toHash().value("Block1").toString().isEmpty()) {
        int b1 = WaveInit.toHash().value("Block1").toString().toInt()*this->width()/400;
...
void WaveBox::mousePressEvent(QMouseEvent *e)
{
    ...
    if (!WaveInit.toHash().value("Block0").toString().isEmpty() && x <= 199) {
        QVariantHash hash = WaveInit.toHash();
        hash.insert("Block0", QString::number(x));
        WaveInit = QVariant::fromValue(hash);
    }
    if (!WaveInit.toHash().value("Block1").toString().isEmpty() && x >= 200) {
        QVariantHash hash = WaveInit.toHash();
        hash.insert("Block1", QString::number(x));
        WaveInit = QVariant::fromValue(hash);
    }
    this->update();
    emit BlockClick(WaveInit);
}
/*********************************END OF FILE**********************************/
