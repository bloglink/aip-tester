 ...
 * brief:      按键功能
*******************************************************************************/
void Resistance::BtnJudge(int id)
{
    switch (id) {
    case Qt::Key_0:
        DatAuto();
 ...
 * brief:      点击
*******************************************************************************/
void Resistance::ItemClick(int r, int c)
{
    switch (c) {
    case 0:
 ...
 * brief:      切换文字
*******************************************************************************/
void Resistance::ItemChange(QString msg)
{
 ...
 * brief:       命令处理
*******************************************************************************/
void Resistance::readMessage(quint16 addr, quint16 cmd, QByteArray msg)
{
 ...
 * brief:      CAN命令处理
*******************************************************************************/
void Resistance::ExcuteCanCmd(QByteArray msg)
{
    if (!Testing)
        return;
    TimeOut = 0;
...
/**
  * @brief  Check resistance status
  * @param  None
  * @retval None
  */
void Resistance::TestCheck()
{
...
/**
  * @brief  Excute status message
  * @param  msg:status message
  * @retval None
  */
void Resistance::TestStatus(QByteArray msg)
{
    if (!isCheckOk) {
        isCheckOk = true;
...
/**
  * @brief  Start resistance test
  * @param  pos:station
  * @retval None
  */
void Resistance::TestStart(quint8 pos)
{
    if (Testing)
        return;
    QByteArray msg;
...
/**
  * @brief  Send judge message
  * @param  None
  * @retval None
  */
void Resistance::TestJudge()
{
    QStringList s;
    s.append("电阻");
    s.append(FileInUse);
 ...
 * brief:       修改显示方式
*******************************************************************************/
void Resistance::TestResult(QByteArray msg)
{
 ...
 * brief:      计算温度补偿
*******************************************************************************/
double Resistance::TestOffset(double t, quint8 num)
{
    double temp;
    double res = 0;
 ...
 * brief:      计算测试档位
*******************************************************************************/
int Resistance::TestGear(int row)
{
    if (row >= Max.size())
        return 7;
    if (row >= Unit.size())
 ...
 * brief:      延时
*******************************************************************************/
void Resistance::TestDelay(int ms)
{
    QElapsedTimer t;
    t.start();
    while(t.elapsed()<ms)
 ...
 * brief:      去除超时处理
*******************************************************************************/
bool Resistance::TestTimeOut(quint16 t)
{
    TimeOut = 0;
    while (Testing) {
        TestDelay(10);
 ...
 * brief:      更新显示
*******************************************************************************/
void Resistance::showEvent(QShowEvent*)
{
    DatInit();
    DatStdd();
}
 ...
 * brief:      判断保存
*******************************************************************************/
void Resistance::hideEvent(QHideEvent *)
{
    if(DatStdd())
        DatSave();
}
